...
var Calendar = (function (_Component) {
  ...
  _createClass(Calendar, [{
    ...
    value: function componentWillReceiveProps(nextProps) {
      ...
      var prevRange = this.props.range;

      if (nextRange && !nextRange['endDate'].isSame(prevRange['endDate'])) {
        var absMonthPrev = prevRange['endDate'].clone().month() * 12;
        var absMonthNext = nextRange['endDate'].clone().month() * 12;
        var absMonthDiff = absMonthNext - absMonthPrev;

        this.changeMonth(-1, null, absMonthDiff);
      } else if (nextRange && nextRange['endDate'] && !nextRange['endDate'].isSame(nextRange['startDate'])) {
    ...
    key: 'changeMonth',
    value: function changeMonth(direction, event, magnitude) {
      if (event) {
      ...
      var newMonth = this.state.shownDate.clone().add(direction * magnitude, 'months');
